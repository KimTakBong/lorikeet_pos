<template>
  <div class="space-y-6">
    ...
    <!-- WA Engine Down Warning -->
    <div v-if="waEngineDown" class="bg-red-900/30 border border-red-700/50 rounded-xl p-4 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <div class="w-3 h-3 rounded-full bg-red-500 animate-pulse"></div>
        <div>
          <div class="font-medium text-red-300">WhatsApp Engine Offline</div>
          <div class="text-sm text-red-400">Start the WA Engine to use WhatsApp features</div>
        </div>
      </div>
      <ButtonSecondary @click="checkStatus" :loading="checkingStatus">Retry</ButtonSecondary>
    </div>

    <!-- Connection Status -->
    <div v-if="!waEngineDown" class="bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-700">
      ...
      <div class="flex items-center justify-between p-4 bg-gray-700/50 rounded-lg">
        <div class="flex items-center gap-3">
          <div :class="['w-3 h-3 rounded-full', status === 'connected' ? 'bg-green-500 animate-pulse' : status === 'qr_required' ? 'bg-yellow-500' : 'bg-red-500']"></div>
          <div>
            <div class="font-medium text-white capitalize">{{ statusText }}</div>
            <div class="text-sm text-gray-400">{{ statusDescription }}</div>
          </div>
        </div>
        <ButtonSecondary @click="checkStatus" :loading="checkingStatus">Refresh</ButtonSecondary>
      </div>
    </div>

    <!-- QR Code Section -->
    <div v-if="!waEngineDown && status === 'qr_required' && qrCode" class="bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-700">
      ...
      <div class="flex flex-col items-center">
        ...
        <p class="text-sm text-gray-400 mt-4 text-center">Open WhatsApp → Settings → Linked Devices → Link a Device<br/>Then scan this QR code</p>
        <ButtonSecondary @click="loadQRCode" class="mt-4">Refresh QR</ButtonSecondary>
      </div>
    </div>

    <!-- Connected -->
    <div v-if="!waEngineDown && status === 'connected'" class="bg-emerald-900/30 border border-emerald-700/50 rounded-xl p-6">
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-emerald-400 mt-1">Your WhatsApp is connected and ready to send messages.</p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-700">
      ...
      <div class="flex gap-3">
        <ButtonDanger @click="confirmLogout" :loading="actionLoading" :disabled="waEngineDown || status === 'connected'">Logout</ButtonDanger>
        <ButtonSecondary @click="confirmDestroy" :loading="actionLoading" :disabled="waEngineDown">Destroy Session</ButtonSecondary>
      </div>
      <p class="text-xs text-gray-400 mt-2">Logout: Sign out but keep session (can reconnect without QR)<br/>Destroy: Delete session completely (will need to scan QR again)</p>
    </div>
    ...
    <div class="bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-700">
      ...
      <div class="space-y-4">
        ...
        <ButtonPrimary @click="sendTestMessage" :disabled="status !== 'connected' || waEngineDown" :loading="sending">Send Test Message</ButtonPrimary>
      </div>
    </div>
  </div>
</template>
...
async function confirmLogout() {
  if (waEngineDown.value) return AlertService.error('WhatsApp Engine is not running');
  if (!await AlertService.confirm('Are you sure you want to logout?')) return;
  actionLoading.value = true;
  try {
    await axios.post(`${waEngineUrl}/api/session/logout`, {}, { headers: { 'X-API-KEY': apiKey } });
...
async function confirmDestroy() {
  ...
  actionLoading.value = true;
  try {
    await axios.post(`${waEngineUrl}/api/session/destroy`, {}, { headers: { 'X-API-KEY': apiKey } });
...
async function sendTestMessage() {
  if (!testPhone.value) return AlertService.error('Please enter a phone number');
  sending.value = true;
  try {
